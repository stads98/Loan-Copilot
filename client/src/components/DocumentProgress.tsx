      ...
      <div className="px-4 py-4 sm:px-6">
        ...
        <div className="mt-6 pt-6 border-t border-gray-200">
          <div className="flex items-center mb-4">
            ...
            <h4 className="text-lg font-medium text-gray-900">Document Checklist</h4>
          </div>
          <p className="text-sm text-gray-600 mb-4">
            Track all required documents. Check them off as you upload each one.
          </p>

          {/* All Required Documents with Status */}
          <div className="space-y-4">
            ...
            <div>
              <h5 className="text-sm font-medium text-blue-800 mb-2 flex items-center">
                ...
                Borrower Documents ({borrowerDocs}/{borrowerRequired})
              </h5>
                ...
                {requiredDocuments.borrower.map((docName, index) => {
                  ...
                  return (
                    <div key={index} className={`flex items-center p-2 rounded-md ${
                      isUploaded ? 'bg-green-50 border border-green-200' : 'bg-amber-50 border border-amber-200'
                    }`}>
                      ...
                      <span className={`text-sm ${
                        isUploaded ? 'text-green-800 font-medium' : 'text-amber-800'
                      }`}>
                        {docName}
                      </span>
            ...
            <div>
              <h5 className="text-sm font-medium text-purple-800 mb-2 flex items-center">
                ...
                Title Documents ({titleDocs}/{titleRequired})
              </h5>
              <div className="grid gap-2">
                {requiredDocuments.title.map((docName, index) => {
                  const isUploaded = documents.some(doc => 
                    doc.category === "title" && 
                    doc.name.toLowerCase().includes(docName.toLowerCase().split(' ')[0])
                  );
            ...
            <div>
              <h5 className="text-sm font-medium text-green-800 mb-2 flex items-center">
                ...
                Insurance Documents ({insuranceDocs}/{insuranceRequired})
              </h5>
              <div className="grid gap-2">
                {requiredDocuments.insurance.map((docName, index) => {
                  const isUploaded = documents.some(doc => 
